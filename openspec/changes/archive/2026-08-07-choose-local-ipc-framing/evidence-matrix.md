...
`7c65677cb518fbc08dead7e26d00f12962a2c6ae` (`fix: preserve HTTP quarantine
replacement ownership`); it includes the final raw probe and HTTP ownership
regressions. The fixtures remain under `tests/fixtures/`; no production `src/`
transport or Pi message/task module is imported.
...
command is the HTTP comparison. The Node guard must exit with status `0`;
`node --version` records the exact runtime after the guard, and the verbose
output must be retained, not replaced by a test-count summary.

For repository checks, run the two focused commands above followed by:

```text
npm run typecheck
npx eslint tests/fixtures/local-ipc tests/fixtures/local-ipc-spike tests/fixtures/local-ipc-http
npm run lint
npx prettier --check openspec/changes/choose-local-ipc-framing/adr.md openspec/changes/choose-local-ipc-framing/evidence-matrix.md openspec/changes/choose-local-ipc-framing/http-comparison.md
npm run format:check
npm run build
npm test
openspec validate choose-local-ipc-framing --type change --strict
git diff --check
```

The canonical test commands and per-test output captured on the available runner
are stored beside this record:
...
- [`cleanup-baseline-delta.txt`](evidence/ubuntu-node-25.0.0/cleanup-baseline-delta.txt)
  — separate pre/post artifact inventory captured around the third raw/HTTP pair;
  it is not a test-run record or a fourth run.
- [`repository-checks.txt`](evidence/ubuntu-node-25.0.0/repository-checks.txt)
  — typecheck, focused/full lint, formatting, build, full test, validation, and
  diff-check output.
- [`openspec-validation.txt`](evidence/ubuntu-node-25.0.0/openspec-validation.txt)
  — strict OpenSpec change validation output.

Each canonical raw/HTTP run file contains the Vitest pass/fail output for its
acceptance scenarios, the Node guard and runtime result, and the final exit
status. `cleanup-baseline-delta.txt` contains only inventory/delta metadata and
run-3 status/count references; it is not a replacement for either run file.
Non-native-platform cases are explicit limitation assertions; they do not claim
native macOS or Windows support.

## Platform matrix

| Platform and runtime         | Raw `node:net` candidate                                                                  | HTTP comparison                                                                                                                    | Cleanup/process evidence                                                                          | Limitation                                                                                                                                                                          |
| ---------------------------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ubuntu/Linux, Node `v25.0.0` | **PASS** — 6 files, 72 tests, including native Unix-socket bind/request/close             | **PASS** — 1 file, 15 tests, including native Unix-socket HTTP bind/request/close                                                  | **PASS** on three repeated focused runs; pre/post artifact delta is empty and child count is zero | Three unrelated pre-existing `/tmp/p2p-*.sock` files and eleven `/tmp/p2p-*` directories were present in the baseline. They were not removed because ownership was not established. |
...
| Child-process cleanup and diagnostics     | `test-helpers.test.ts`: bounded terminate/escalate/close, null close state, reused child generations, late errors, bounded output; `raw-net.process.test.ts`: abrupt child termination and cleanup                                                                                                               | **PASS**                                                                                 |
| POSIX stale endpoint safety               | `raw-net.process.test.ts`: safely probes an abrupt stale socket, rejects an inconclusive bare-close probe, preserves a live listener, rejects unowned roots, and refuses non-socket unlink; `raw-net.unit.test.ts`: preserves a moved replacement when another listener claims the vacant endpoint before relink | **PASS** on Linux; macOS unavailable                                                     |
...
8, raw lifecycle 23, raw unit 4, process lifecycle 6, and helper lifecycle 22,
for 72 passing tests in 6 files.
...
| Endpoint replacement safety           | `recovers an unchanged moved regular-file replacement without following it`, the corresponding symlink case, and `preserves a live moved replacement when the endpoint is claimed before relink`; Windows branches assert the named-pipe limitation | **PASS**                                                                             |
...
The canonical HTTP run files `http-run-1.txt`, `http-run-2.txt`, and
`http-run-3.txt` each record 15 passing tests in one file. The fixture's focused
report also records that no slow-drip response peer was exercised for HTTP;
therefore no HTTP slow-drip result is claimed.

## Repeated-run cleanup evidence

The canonical test records are `raw-run-1.txt`, `raw-run-2.txt`, and
`raw-run-3.txt` for raw (72 tests each), plus `http-run-1.txt`,
`http-run-2.txt`, and `http-run-3.txt` for HTTP (15 tests each).
`cleanup-baseline-delta.txt` is a separate cleanup inventory captured around the
third pair; it runs the raw command and then the HTTP command with a pre-run and
post-run inventory of:

- POSIX socket files (`/tmp/p2p-*.sock`);
- quarantine entries (`/tmp/p2p-*.sock.cleanup-*`);
...
The baseline contained 3 socket files, 0 quarantine entries, 11 temporary
directories, and 0 fixture children. The post-run-minus-pre-run delta and the
reverse delta were empty; both focused commands exited `0`, and the recorded
`CLEANUP_COMMAND_EXIT_STATUS` and `EXIT_STATUS` are `0`. This proves that the
repeated runs introduced no new owned endpoint, quarantine file, temporary
directory, or helper process. The pre-existing baseline entries were deliberately
left untouched because the fixture must never blindly unlink an arbitrary path.
...
live replacement preservation, and child close observation. Windows pipe cleanup
cannot be inferred from this Ubuntu inventory and remains a required native
Windows run.

## Evidence status

- Ubuntu/Linux raw candidate: **PASS** — `raw-run-1.txt`, `raw-run-2.txt`, and
  `raw-run-3.txt` each contain all 72 passing scenarios.
- Ubuntu/Linux HTTP candidate: **PASS** — `http-run-1.txt`, `http-run-2.txt`, and
  `http-run-3.txt` each contain all 15 passing scenarios.
- Ubuntu/Linux repeated cleanup delta: **PASS** — `cleanup-baseline-delta.txt`
  records no new artifacts or fixture children.
