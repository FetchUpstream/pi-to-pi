...
The normal default export SHALL register the four Pi-to-Pi communication tools without requiring callers to construct or inject a `PiAdapter`, `MessageRouter`, `AgentCardRegistry`, or `LocalIpcTransport`. On `session_start`, it SHALL create one runtime-scoped composition owning the canonical session/runtime identity, room, generated endpoint, bound `LocalIpcTransport`, `AgentCardRegistry`, `MessageRouter`, `PiAdapter`, and immutable generation. The extension factory SHALL not allocate these resources.
...
- **THEN** it exposes `p2p_peers`, `p2p_send`, `p2p_reply`, and `p2p_status` and creates a current production runtime without an externally supplied adapter
