...
export interface TaskStoreOptions {
  ...
  /** Injecting a clock makes absolute deadline behavior deterministic in tests. */
  readonly now?: () => number;
...
function normalizeOwner(input: TaskStoreCreateInput): TaskOwner {
  const owner = input.owner ?? input.sender;
  let runtimeId: string | undefined;
  let sessionId: string | undefined;

  if (typeof owner === 'string') {
    runtimeId = owner;
  } else if (owner !== undefined && typeof owner === 'object') {
    runtimeId = owner.runtimeId;
    sessionId = owner.sessionId;
  }

  runtimeId ??= runtimeIdOf(input.ownerRuntimeId);
  runtimeId ??= runtimeIdOf(input.senderRuntimeId);
...
export class TaskStore {
  ...
  constructor(options: TaskStoreOptions = {}) {
    ...
    this.nowFn = options.now ?? options.clock ?? (() => Date.now());
    this.scheduleTimer =
  ...
  private scheduleExpiry(record: StoredTask): void {
    const delayMs = Math.max(0, record.deadlineMs - this.currentNow());
    if (delayMs === 0) {
  ...
  private expireIfDue(record: StoredTask): void {
    if (!isTerminal(record.snapshot.state) && this.currentNow() >= record.deadlineMs) {
      this.expireRecord(record);
    }
  }
